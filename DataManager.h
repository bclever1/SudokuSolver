...
#if 0
	static int GetBoardValue(int theBoardIdx)
	{
		std::lock_guard<std::mutex> guard(myMutex);
		return (*myData)[theBoardIdx].get()->getBoardValue();
	}


	static bool GetSolved(int theBoardIdx)
	{
		std::lock_guard<std::mutex> guard(myMutex);
		return (*myData)[theBoardIdx].get()->Solved();
	}

	static void Reduce(int theBoardIdx, Board::SquareGroupType_e theGrpType, int theItemNum)
	{
		std::lock_guard<std::mutex> guard(myMutex);
		(*myData)[theBoardIdx].get()->Reduce(theGrpType, theItemNum);
	}

	static void RemoveStrandedSingles(int theBoardIdx, Board::SquareGroupType_e theGrpType, int theItemNum)
	{
		std::lock_guard<std::mutex> guard(myMutex);
        (*myData)[theBoardIdx].get()->CheckForStrandedSingles(theGrpType, theItemNum);
	}

	static void RemoveNakedPairs(int theBoardIdx, Board::SquareGroupType_e theGrpType, int theItemNum)
	{
		std::lock_guard<std::mutex> guard(myMutex);

		(*myData)[theBoardIdx].get()->CheckForNakedPairs(theGrpType, theItemNum);
	}

	static vector<int> GetSquareValues(int theBoardIdx, int i, int j) 
	{
		return (*myData)[theBoardIdx].get()->GetSquareValues(i, j);
	};
#endif
